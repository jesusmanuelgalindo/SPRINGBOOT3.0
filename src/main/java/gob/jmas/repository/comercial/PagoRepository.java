...
public interface PagoRepository extends JpaRepository<Pago, Integer> {
       ...
       @Query(value = "select pagos.Pagos_Id as id,usua_pa as cuenta,(select nomb_us from usuarios where clav_us =usua_pa ) as nombre," +
               ...
               " date(fech_pa) as fechaDePago ,conc_pa as clave,(select conceptos.c_claveProvServ from conceptos where clav_cc=conc_pa) as conceptoFiscal," +
               "(select desc_cc from conceptos where clav_cc=conc_pa) as concepto, mont_pa as monto, tasa_pa as tasa from pagos where" +
